module.exports = {
    getMyRequests: "SELECT s.id, s.codigo, e.titulo, DATE_FORMAT(s.fecha_envio, '%d/%m/%Y') AS fecha_envio, " +
    ...
    ,
    getMyRequestsDetails: "SELECT s.codigo, DATE_FORMAT(s.fecha_envio,'%d/%m/%y') AS fecha_envio, e.titulo, " +
    ...
    ,
    getRequestAdmin: "SELECT s.id, s.codigo, e.titulo, DATE_FORMAT(s.fecha_envio, '%d/%m/%Y') AS fecha_envio " +
                    "FROM solicitudes AS s JOIN eventos AS e ON s.id_evento = e.id WHERE s.estado = 3 GROUP BY " +
                    "s.fecha_envio"
    ,
    getRequestsDetailsAdmin: "SELECT s.codigo, DATE_FORMAT(s.fecha_envio,'%d/%m/%y') AS fecha_envio, e.titulo, " +
                            ...
                            "WHERE s.id = ?"
    ,
    approvedRequest: "UPDATE solicitudes SET estado = 2, observaciones = ? WHERE id = ?"
    ,
    rejectedRequest: "UPDATE solicitudes SET estado = 1, observaciones = ? WHERE id = ?"
    ,
