module.exports = {
    getEvents: "SELECT e.id,e.logo, e.titulo, DATE_FORMAT(e.fecha_inicio, '%W, %d de %b del %Y') AS " +
                ...
                "AND s.estado = 2"
    ,
    getEventDetails: "SELECT e.logo AS logo, e.titulo, tco.descripcion AS tipo_coordinador, e.nombre_coordinador, " +
    ...
    ,
    getMyEvents: "SELECT e.id, e.titulo,e.logo as logo, DATE_FORMAT(e.fecha_inicio, '%W, %d de %M del %m') " +
    ...
    ,
    getEventsByPrices: "SELECT e.id,e.logo, e.titulo, DATE_FORMAT(e.fecha_inicio, '%W, %d de %b del %Y') AS " +
                        "fecha_inicio, DATE_FORMAT(e.hora_inicio, '%h:%i %p') AS hora_inicio, e.precio_inscripcion " +
    ...
    ,
    getEventsByCategory: "SELECT e.id,e.logo, e.titulo, DATE_FORMAT(e.fecha_inicio, '%W, %d de %b del %Y') AS " +
                        "fecha_inicio, DATE_FORMAT(e.hora_inicio, '%h:%i %p') AS hora_inicio, e.precio_inscripcion " +
    ...
    ,
    getInscribed: "SELECT COUNT(id) AS Inscritos FROM inscripciones WHERE id_evento = ?"
    ,
    getParticipants: "SELECT Participantes FROM eventos WHERE id = ?"
    ,
    updateStatusToComplete: "UPDATE solicitudes SET Estado = 4 WHERE id_evento = ?"
}