...
module.exports = {
    ...
    executeInscription: async (req, res) => {
        ...
        const inscription = { id_evento: id, id_usuario: req.user.id, certificado, voucher };

        await db.query(query.insertInscription, [inscription]);

        res.status(200).json({ data: null, message: "Inscription Registered to this event" });
    },
    ...
    executeEventRegister: async (req, res) => {
        ...
        const { titulo, tipo_coordinador, nombre_coordinador, tipo_evento, fecha_inicio, fecha_fin, hora_inicio,
                duracion, tipo_inscripcion, precio_inscripcion, descripcion, tipo_certificado, precio_certificado,
                tipo_ambiente, participantes, logo } = req.body;

        const newEvent = { titulo, tipo_coordinador, nombre_coordinador, id_coordinador, tipo_evento, fecha_inicio,
                            fecha_fin, hora_inicio, duracion, tipo_inscripcion, precio_inscripcion, descripcion,
                            tipo_certificado, precio_certificado, tipo_ambiente, participantes, logo };

        const result = await db.query(query.insertEvent, [newEvent]);

        const codigoRequest = "FISI2022-" + result.insertId;

        const newRequest = { id: null, codigo: codigoRequest, estado: 3, id_evento: result.insertId };

        await db.query(query.insertRequest, [newRequest]);

        res.status(200).json({ data: null, message: "Event Registered and Request Created" });
    },
