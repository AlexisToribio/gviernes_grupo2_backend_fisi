...
module.exports = {
    ...
    updateUser: async (req, res) => {
        ...
        const { nombres, apellidos, email, edad, celular, sexo, ocupacion, foto } = req.body;
        
        const newUser = { nombres, apellidos, email, edad, celular, sexo, ocupacion, foto };

        await db.query(query.updateUser, [newUser, id], (err, result) => {

            if (err) {

                return res.status(500).json({ data: null, message: "Error when updating the user data" });
            }
            else {

                return res.status(200).json({ data: null, message: "Update was successful" });
            }
        });
    },
    executeSignIn: async (req, res) => {
        ...
        if (rows.length > 0) {
            ...
            if (validate) {
                ...
                return res.json({ token, user: { nombres, apellidos, foto, tipo_usuario }, message: "Login successful" });
            } 
            else {
                res.status(400).json({ data: null, message: "Your password is incorrect" });
            }
        } 
        else {
    ...
    executeSignUp: async (req, res) => {
        ...
        await db.query(query.insertUser, [newUser], (err, result) => {
            ...
            if (err) {
                ...
                return res.status(200).json({ data: null, message: "Register was successful" });
            }
        });
    }
};